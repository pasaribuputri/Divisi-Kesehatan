    ...
    if(typeof data !== 'string'){
        for(let i=0;i<data.length;i++){
            ...
            anchorDetail.addEventListener('click',()=>{
                location.href = `/divkes/data-sehat/edit-data-sehat/?nama=${data[i].nama}`
            })
...
async function showAllDataSehat(){
    await fetch("http://localhost:3000/api/kondisiMahasiswa/getDataSehat",{
        method: 'GET',
        headers: {
...
async function showPencarianDataSehat(cari){
    await fetch(`http://localhost:3000/api/kondisiMahasiswa/getDataSehatByNama/${cari}`,{
        method: 'GET',
        headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
        }
